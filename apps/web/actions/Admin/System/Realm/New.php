 ...
 * Index クラス
 */
class Admin_System_Realm_New extends AppAdminSystemAction
{
    /**
     * デフォルトアクション処理
     *
    ...
    public function execute(SyL_ContextAbstract $context, SyL_Data $data)
    {
        ...
        $group_root_url = $data->get('App.Admin.url_system_base') . 'realm/';
        $this->addBreadcrumbs($data, 'アクセス範囲管理', $group_root_url);
    ...
    protected function executeAjax(SyL_ContextAbstract $context, SyL_Data $data)
    {
        ...
        if (!$view->isErrors()) {
            ...
            BlAbstract::createInstance('realm')->registerRealm($view->getValue('realm_name'), $view->getValue('realm'), $valid_flag, $i_id);
            $result['valid'] = true;
        } else {
            $result['valid'] = false;
    ...
    {
        $form_config = SyL_FormConfigAbstract::createInstance(ConfigPath::getFormRealmNewFile());
